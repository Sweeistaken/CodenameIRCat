...
def session(connection, client, ip, isssl=False):
    setproctitle.setproctitle(f"Codename IRCat: {client[0]} Unknown connection")
                    ...
                    for text in textt.replace("\r", "").split("\n"):
                        ...
                        if command == "NICK" and not finished:
                            ...
                            if not isalphanumeric(pending):
                                ...
                                setproctitle.setproctitle(f"Codename IRCat: {client[0]} as {pending} logging in")
                                already_set = True
                        elif command == "USER":
                            if not ready:
                            ...
                            setproctitle.setproctitle(f"Codename IRCat: {client[0]} as {pending}")
                            del alreadyNotified
...
def tcp_session(sock, ip_to):
    setproctitle.setproctitle(f"Codename IRCat: TCP listener for {ip_to}")
    while opened:
        try:
...
def ssl_session(sock, ip_to):
    setproctitle.setproctitle(f"Codename IRCat: SSL listener for {ip_to}")
    while opened:
        try:
